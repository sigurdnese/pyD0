...
def get_zdc_corrected_yield(N_vec, N_err_vec, C, neutron_class=None, do_print=False):
    if neutron_class=='0nXn':
        class_idx = 1
    elif neutron_class=='Xn0n':
        class_idx = 2
    else:
        raise Exception("'class' must be '0nXn' or 'Xn0n'!")

    Cinv = np.linalg.inv(C)
    M_vec = Cinv @ N_vec
    M = M_vec[class_idx]
    N = N_vec[class_idx]
    N_cov = np.diag(N_err_vec**2)
    M_cov = Cinv @ N_cov @ Cinv.T
    M_err = np.sqrt(M_cov.diagonal()[class_idx])
    N_err = N_err_vec[class_idx]
    if do_print:
        print(f"N = {N} +- {N_err:.0f}")
        print(f"M = {M} +- {M_err:.0f} ({100*M_err/M:.3f}%)")

    # Define a function to return the correction factor, to facilitate numerical error propagation
    def correction_factor(Nv):
        Mv = Cinv @ Nv
        return Mv[class_idx] / Nv[class_idx]

    c, cov_c = propagate(correction_factor, N_vec, N_cov)

    # correction_factor_old = M/N
    # # Binomial error formula from ROOT TH1.Divide
    # correction_factor_err_old = np.sqrt(np.abs( ( (1. - 2.* M / N) * M_err**2  + M**2 * N_err**2 / N**2 ) / N**2 ))

    return {
        'M' : M,
        'M_err' : M_err,
        'correction_factor' : c,
        'correction_factor_err' : np.sqrt(cov_c)
    }

def get_neutron_class_vector(hist, do_print=False):
    """
    Returns the vector of observed neutron class counts N = (0n0n, 0nXn, Xn0n, XnXn)
    using ZNA time vs ZNC time histogram hist
    """
    bin_low = hist.GetXaxis().FindBin(-2)
    bin_high = hist.GetXaxis().FindBin(2) - 1
    bin_max = hist.GetXaxis().GetNbins()
    
    N_XnXn = hist.Integral(bin_low, bin_high, bin_low, bin_high)
    N_0nXn = hist.Integral(0, bin_low-1, bin_low, bin_high) + hist.Integral(bin_high+1, bin_max+1, bin_low, bin_high)
    N_Xn0n = hist.Integral(bin_low, bin_high, 0, bin_low-1) + hist.Integral(bin_low, bin_high, bin_high+1, bin_max+1)
    N_0n0n = hist.Integral(0, bin_low-1, 0, bin_low-1) + hist.Integral(0, bin_low-1, bin_high+1, bin_max+1) + hist.Integral(bin_high+1, bin_max+1, 0, bin_low-1) + hist.Integral(bin_high+1, bin_max+1, bin_high+1, bin_max+1) 
    N_vector = np.array([N_0n0n, N_0nXn, N_Xn0n, N_XnXn])
    if do_print:
        for name, num in zip(['0n0n', '0nXn', 'Xn0n', 'XnXn'], N_vector):
            print(f"{num:.3E} {name} events")
    return N_vector
...
def correct_efficiency_ir_dependence(hist_yield_per_lumi, hist_mc_rec_ir, hist_mc_gen_ir, savefig=None):
    ...
    ax_bottom.tick_params(axis="y", direction='in')
    ...
    ax_top.plot(ir_axis, y_mc_fit, color='C3', label='$F_\\mathrm{MC}$')
    ax_top.fill_between(ir_axis, y_mc_fit-y_mc_fit_errprop, y_mc_fit+y_mc_fit_errprop, facecolor='C3', alpha=0.5)

    ax_top.plot(ir_axis, y_data_norm, color='C1', label='$F_\\mathrm{data}$')
    ...
    order = [2,0,3,1]
    ax_top.legend([handles[idx] for idx in order], [labels[idx] for idx in order], frameon=False, loc='lower left')
    ax_top.set_ylim(bottom=0.005, top=0.095)
    ...
    ax_bottom.plot(ir_axis, R_ir, color='C4', label='Correction factor for MC efficiency ($F_\\mathrm{data}/F_\\mathrm{MC}$)')
    ...
    tmp_dict = apply_ir_rescaling(hist_mc_rec_ir, hist_mc_gen_ir, theta, cov_theta, ir0, trim, n_data)
    eff_rescaled_integrated = tmp_dict['eff_rescaled']
    eff_rescaled_integrated_err_total = tmp_dict['err']
    eff_rescaled_integrated_err_stat = tmp_dict['err_stat']
    eff_rescaled_integrated_err_fit = tmp_dict['err_fit']
...
def apply_ir_rescaling(hist_mc_rec_ir, hist_mc_gen_ir, theta, cov_theta, ir0, trim, n_data):
    ...
    eff_mc_arr = np.where(N_gen_mc_arr==0, 0, N_rec_mc_arr / N_gen_mc_arr)
    ...
    # Raw MC efficiencies, bin by bin. Treated as deterministic wrt the fit parameter error propagation
    eff_mc_arr = np.where(N_gen_mc_arr==0, 0, N_rec_mc_arr / N_gen_mc_arr)
    # Cap the efficiency at 1 and print warning
    if np.any(eff_mc_arr > 1):
        print("Efficiency larger than 1 at the following bin indices:")
        print(np.where(eff_mc_arr>1)[0]+1)
    eff_mc_arr = np.where(eff_mc_arr > 1, 1, eff_mc_arr)
    eff_var_stat_arr = np.where(N_gen_mc_arr==0, 0, eff_mc_arr * (1.0 - eff_mc_arr) / N_gen_mc_arr)
    ...
    return {
        'eff_rescaled' : eff_rescaled_integrated,
        'err'          : eff_rescaled_integrated_err_total,
        'err_stat'     : eff_rescaled_integrated_err_stat,
        'err_fit'      : eff_rescaled_integrated_err_fit,
        'R_arr'        : R_arr
    }
...
def get_histograms(directory, runList, fullHistogramNames, histogramNamesfileStructure = FileStructures.FLAT, fileName = 'AnalysisResults.root', fallbackDir = None):
    ...
    histograms = {}
    s = f"Getting histograms from {directory}"
    dirs = [directory]
    if fallbackDir is not None:
        s += f" and {fallbackDir}"
        dirs.append(fallbackDir)
    print(f"{s}...")
    if depth == 3:
        for irun, run in enumerate(runList):
            ...
            if (histogramNamesfileStructure == FileStructures.FLAT):
                filePaths = [f"{d}/AnalysisResults_run{run}.root" for d in dirs]
            elif (histogramNamesfileStructure == FileStructures.RUNDIRS):
                filePaths = [f"{d}/{run}/{fileName}" for d in dirs]
            else:
                raise Exception("Not a valid FileStructure!")
            with utils.uproot_open_fallback(filePaths) as file:
                for dirName in namesDict.keys():
                    try:
                        dir = file[dirName]
                    except:
                        raise Exception(f"Couldn't get '{dirName}' from file '{filePaths}'!")
                            ...
                            for ii, iitem in enumerate(dir[i]):
                                if iitem.member("fName") in namesDict[dirName][item.member("fName")]:
                                    ...
                                    if tmpHistPr.GetEntries() == 0:
                                        print(f"WARNING: Histogram '{fullName}' in '{filePaths}' has no entries!")
...
class FitResult():
    ...
    def create_fit_results(self, reflectedLowerMass, reflectedUpperMass, bin, 
        ...
        try:
            ...
            print("WARNING: Did not manage to calculate the chi2/ndf for the fit function!")
            self.fitChi2Ndf = 0
        self.massBinWidth = self.histogram.GetBinWidth(1)
        self.fitMu = fitFunc.GetParameter('Mean')
        self.fitSigma = fitFunc.GetParameter('Sigma')
        self.nSignal = fitFunc.GetParameter(0) / self.massBinWidth
        self.nCombBackground = backgroundFunc.Integral(self.fitMu - 3*self.fitSigma, self.fitMu + 3*self.fitSigma) / self.massBinWidth
        if doPrint:
        ...
        self.nReflBackground = dataReflFunc.GetParameter(0) / self.massBinWidth
        if self.nSignal != 0:
            # Symmetric Hesse error
            self.relativeStatError = fitFunc.GetParError(0) / (self.massBinWidth * self.nSignal)
            # Asymmetric MINOS errors
            self.relativeStatErrorLower = np.abs(self.result.LowerError(0)) / (self.massBinWidth * self.nSignal)
            self.relativeStatErrorUpper = np.abs(self.result.UpperError(0)) / (self.massBinWidth * self.nSignal)
        else:
            # Symmetric Hesse error
            self.relativeStatError = 0
            # Asymmetric MINOS errors
            self.relativeStatErrorLower = 0
            self.relativeStatErrorUpper = 0
        signalIntegral3Sigma = signalFunc.Integral(self.fitMu - 3*self.fitSigma, self.fitMu + 3*self.fitSigma) / self.massBinWidth
        reflIntegral3Sigma = dataReflFunc.Integral(self.fitMu - 3*self.fitSigma, self.fitMu + 3*self.fitSigma) / self.massBinWidth
        corrIntegral3Sigma = (corrBkgFunc.Integral(self.fitMu - 3*self.fitSigma, self.fitMu + 3*self.fitSigma) / self.massBinWidth) if corrBkgFunc is not None else 0
...
class PtBin():
    ...
    def draw(self, createCanvas=True):
        ...
        if hasattr(self, 'nominalFitResult'):
            ...
            self.nominalFitResult.histogram.Draw("same E")
            self.nominalFitResult.histogram.SetStats(0)
        else:
            self.nominalFitResult.histogram.Draw("E")
            self.nominalFitResult.histogram.SetStats(0)
        if createCanvas:
            self.canvas.Draw()
    ...
    def draw_systematic_fit_variations(self, showFits=False):
        ...
        self.histSystematicSigmaVsTrial = r.TH1D("histSystematicSigmaVsTrial", f"{self.lowerPt} < p_{{T}} < {self.upperPt} GeV/c", len(self.fitResults), 0, len(self.fitResults))
        self.histSystematicSigmaVsTrial.GetXaxis().SetTitle("Trial")
        self.histSystematicSigmaVsTrial.GetYaxis().SetTitle("Signal width")
        ...
        for i, fitResult in enumerate(self.fitResults):
            ...
            self.histSystematicSigmaVsTrial.SetBinContent(i+1, fitResult.fitSigma)
            self.histSystematicSigmaVsTrial.SetBinError(i+1, fitResult.fitFunc.GetParError('Sigma'))
            if showFits:
                self.canvasSystematicFitVariations.cd(panelIndex)
                fitResult.histogram.Draw("E")
                fitResult.histogram.SetStats(0)
        ...
        self.histSystematicSigmaVsTrial.SetMarkerStyle(20)
        self.histSystematicSigmaVsTrial.Draw("P")
        self.lineNominalSigma = r.TLine(self.histSystematicSigmaVsTrial.GetXaxis().GetXmin(), self.nominalFitResult.nSignal, self.histSystematicSigmaVsTrial.GetXaxis().GetXmax(), self.nominalFitResult.fitSigma)
        self.lineNominalSigma.SetLineColor(r.kRed)
        self.lineNominalSigma.SetLineStyle(2)
        self.lineNominalSigma.SetLineWidth(2)
        self.lineNominalSigma.Draw('same')
        self.canvasSystematicFitVariations.cd(panelIndex+3)
        self.histSystematicYieldDistribution.Draw("hist")
        self.textBoxRmsOverMean = r.TPaveText(0.6, 0.5, 0.9, 0.6, "NDC")
        self.textBoxRmsOverMean.AddText(f"{len(self.fitResults)} trials saved")
    ...
    def draw_fit_result(self, index):
        ...
        self.fitResults[index].histogram.Draw("same E")
        self.fitResults[index].histogram.SetStats(0)
...
class Analysis():
    ...
    def __init__(self, pathTableMaker, doData = True, doMc = True,
                 dirData = None, dirMc = None, dirGen = None, dirRefl = None, dirDataRecoverFailedJobs = None,
        ...
        self.dirDataRecoverFailedJobs = dirDataRecoverFailedJobs
        ...
        cutNames = {
            ...
            "pairCutName"          : "",
            "mcCommonTrackCutName" : "singleGapTrackCuts4"
        }
        ...
        self.mcCommonTrackCutName = cutNames["mcCommonTrackCutName"]
        print(f"mcCommonTrackCutName = {self.mcCommonTrackCutName}")
        if self.pairCutName != "":
        ...
        if doData:
            self.prepare_histograms_data()
        if doMc:
            self.prepare_histograms_mc()
    ...
    def prepare_histograms_mc(self):
        self.groupNameD0Generated = "MCTruthGenAfterBcCuts_D0FS"
        self.groupNameD0PtMatched = f"PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}_KPiFromD0FS"
        ...
        baseNamesMc = ["noTrackCut:noTrackCut", f"{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}", f"{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}"]
        ...
        fullNamesMc.append(f"analysis-asymmetric-pairing/output;1/PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}_KPiFromD0FS/MyYPtInteractionRateHisto")
        ...
        try:
            ...
            self.histD0YPtMatched = self.dictRecHists[f"analysis-asymmetric-pairing/output;1/PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}_KPiFromD0FS/MyYPtInteractionRateHisto"]
        ...
        for i, bin in enumerate(self.ptBins):
            # Create an array of slices of MC reflected mass vs pT histogram
            lowerBin = self.histD0MassPtReflected.GetYaxis().FindBin(bin.lowerPt)
            upperBin = self.histD0MassPtReflected.GetYaxis().FindBin(bin.upperPt) - 1
    ...
    def prepare_histograms_data(self):
        ...
        # Get data histograms
        tmpDict = get_histograms(self.dirData, self.runList, [fullNameHistD0MassPt, fullNameHistD0MassPtY, fullNameHistD0MassPtIsGap, fullNameHistVtxNContribAfterCuts, fullNameHistVtxNContribRealAfterCuts, fullNameHistMultiDimA, fullNameHistMultiDimC], histogramNamesfileStructure=self.dataFileStructure, fallbackDir=self.dirDataRecoverFailedJobs)
        try:
    ...
    def systematic_fit_variation(self, bin, backgroundDict, lowers, uppers, rebins, signalMuRange, signalSigmaRange, useCorrBkgTemplate=False, maxChi2=2.0, doSigmaVariations=False, doPrint=False):
        if useCorrBkgTemplate and np.array(lowers).min() < utils.lowest_safe_hf_mass(self.ptBins[bin].lowerPt):
            raise Exception(f"The lowest mass range value {np.array(lowers).min():.2f} is outside the D0->Kpipi0 template range!")
        if len(self.ptBins[bin].fitResults) != 0:
            print(f"WARNING: This bin already has {len(self.ptBins[bin].fitResults)} trials, these will now be overwritten!")
            self.ptBins[bin].fitResults = []
        reflRatioFactors = [1.0, 0.9, 1.1]
        signalWidthFactors = [1.0]
        if doSigmaVariations:
            signalWidthFactors.append(0.95)
            signalWidthFactors.append(1.05)
        nTrials = len(backgroundDict) * len(lowers) * len(uppers) * len(reflRatioFactors) * len(signalWidthFactors) * len(rebins)
        counter = 0
                        ...
                        for rebin in rebins:
                            for signalWidthFactor in signalWidthFactors:
                                ...
                                if (lower == self.ptBins[bin].nominalFitResult.lowerMass and
                                    ...
                                    continue
                                infoString += f"{backgroundFunction.__name__}, fit range [{lower}, {upper}], reflRatio factor = {reflRatioFactor}, rebin={rebin}, "
                                fitResult = self.fit_inv_mass(bin, backgroundFunction, fitRange=[lower, upper], doPrint=doPrint, isNominal=False, 
                                                              signalMuRange=signalMuRange, signalSigma=sigma, signalSigmaRange=signalSigmaRange, rebin=rebin,
    ...
    def fit_reflected(self, bin, fitResult, doPrint=True):
        ...
        reflFunc.SetParLimits(3, 0.005, 0.2)
        reflFunc.SetParLimits(5, 0.005, 0.2)
        if doPrint:
    ...
    def fit_inv_mass(self, bin, backgroundFunction, fitRange = [1.64, 2.08], doPrint=True, isNominal=True, 
                     ...
                     reflRatioFactor=1.0,
                     rebin=1):
        # Create clone of mass histogram so that it can be rebinned as specified
        massPtSlice = self.ptBins[bin].massPtSlice.Clone(f"invMassToFit_bin{bin}")
        massPtSlice.Rebin(rebin)
        # Create the results object to save info on this fit
        fitResult = FitResult(fitRange[0], fitRange[1], backgroundFunction.__name__, bin)
        fitResult.histogram = massPtSlice
        if doPrint:
        ...
        massPtSlice.Fit(simpleFitFunc, "LME0" + ("Q" if not doPrint else ""), "", simpleFitRange[0], simpleFitRange[1])
        fitFunc.SetParameter(0, simpleFitFunc.GetParameter(0)) # Signal yield
        fitFunc.SetParameter(1, simpleFitFunc.GetParameter(1)) # Mean
        fitFunc.SetParameter(2, simpleFitFunc.GetParameter(2)) # Sigma
        # Set the background parameters to safe defaults (avoid division by 0)
        ...
        if doPrint:
            ...
            for i in range(backgroundNPar):
                print(f"par[{3 + 6 + i}] = {fitFunc.GetParameter(3 + 6 + i)}")
        # Obtain a shadowed histogram with signal region removed, and do an initial fit to the background
        massPtSliceShadowed = massPtSlice.Clone(f"invMassShadowed_bin{bin}")
        for i in range(1, massPtSliceShadowed.GetNbinsX() + 1):
            binCenter = massPtSliceShadowed.GetBinCenter(i)
            if binCenter > shadowRange[0] and binCenter < shadowRange[1]:
                massPtSliceShadowed.SetBinContent(i, 0.0)
                massPtSliceShadowed.SetBinError(i, 0.0)
        # Fix the signal yield, mean and sigma to the previously obtained values
        fitFunc.FixParameter(0, simpleFitFunc.GetParameter(0)) # Signal yield
        fitFunc.FixParameter(1, simpleFitFunc.GetParameter(1)) # Mean
        fitFunc.FixParameter(2, simpleFitFunc.GetParameter(2)) # Sigma
        massPtSliceShadowed.Fit(fitFunc, "LME0" + ("Q" if not doPrint else ""), "", fitRange[0], fitRange[1])
        if doPrint:
            print("After fit to shadowed histogram, the combinatorial background function parameters are:")
            for i in range(backgroundNPar):
                print(f"par[{3 + 6 + i}] = {fitFunc.GetParameter(3 + 6 + i)}")
        ...
        ptr = massPtSlice.Fit(fitFunc, "LMES0" + ("Q" if not doPrint else ""), "", fitRange[0], fitRange[1])
    ...
    def calculate_efficiency(self):
        ...
        self.efficiencyFine.Divide(self.efficiencyFine, self.histD0PtGenerated, 1, 1, 'B')
        ...
        self.efficiency.Divide(self.efficiency, self.histD0PtGeneratedFinalBins, 1, 1, 'B')
        if self.rapidityGapSelectionEfficiency is not None:
    ...
    def reweight_efficiency(self):
        ...
        self.efficiencyReweighted.Divide(self.efficiencyReweighted, histDenominator, 1, 1, 'B')
        ...
        # For now, just inherit the error from the pre-reweighting efficiency. Error due to the reweighting procedure should be evaluated as a systematic
        for i in range(1, self.efficiency.GetNbinsX()+1):
            self.efficiency.SetBinError(i, self.efficiencyWithoutReweighting.GetBinError(i))
    ...
    def calculate_factorized_efficiencies(self, allowOneDimHistograms=False):
        ...
        self.histD0PtMatchedInSelEventAfterTrackCuts = self.get_reconstructed_mc_histogram(f"analysis-asymmetric-pairing/output;1/PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}_KPiFromD0FS",
        ...
        if not hasattr(self, 'histD0PtMatchedFinalBins'):
            self.histD0PtMatchedInSelEventAfterTrackCutsAndPairCuts = self.get_reconstructed_mc_histogram(f"analysis-asymmetric-pairing/output;1/PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}_KPiFromD0FS",  "PtMcMatchedInSelEventAfterTrackCutsAndPairCuts", "Reconstructed, matched D0->Kpi in selected event, selected tracks, selected pairs")
        else:
    ...
    def rescale_ir_dependence(self, theta, cov_theta, n_data=2):
        ...
        hist_rec_3d = self.dictMcHists[f"analysis-asymmetric-pairing/output;1/PairsBarrelSEPM_{self.kaonLegCutName}:{self.pionLegCutName}_{self.mcCommonTrackCutName}{self.pairCutName}_KPiFromD0FS/MyYPtInteractionRateHisto"]
        ...
        for i, bin in enumerate(self.ptBins):
            ...
            hist_rec_ir = hist_rec_2d.ProjectionY(f"hist_rec_{bin.lowerPt}_{bin.upperPt}", firstxbin=lowerBin, lastxbin=upperBin)
            hist_gen_ir = hist_gen_2d.ProjectionY(f"hist_gen_{bin.lowerPt}_{bin.upperPt}", firstxbin=lowerBin, lastxbin=upperBin)
            # hist_gen_ir, hist_rec_ir, n_trimmed = utils.trim_trailing_zeros_root(hist_gen_ir_orig, hist_rec_ir_orig)
            tmp_dict = apply_ir_rescaling(hist_rec_ir, hist_gen_ir, theta, cov_theta, 0, 0, n_data)
            eff_i = tmp_dict['eff_rescaled']
            eff_err_i = tmp_dict['err'] 
            print(f"=== Bin {bin.index} ===")
            print(f"Relative err_stat = {100*tmp_dict['err_stat']/eff_i:.3f}%")
            print(f"Relative err_fit = {100*tmp_dict['err_fit']/eff_i:.3f}%")
            R_i = tmp_dict['R_arr'] # By design this array is the same for every pT bin
            self.efficiency.SetBinContent(i+1, eff_i)
            self.efficiency.SetBinError(i+1, eff_err_i)
            self.efficiency.SetBinError(i+1, eff_err_i)

        # Now that the IR corrected efficiency has been obtained, calcualte the corrected spectrum
        self.calculate_corrected_spectrum()
        
        # Create rescaled 2d reconstruction lvl histogram
        hist_rec_2d_rescaled = hist_rec_2d.Clone()
        hist_rec_2d_rescaled.SetName('hist_rec_2d_rescaled')
        ny = hist_rec_2d_rescaled.GetNbinsY()
        nx = hist_rec_2d_rescaled.GetNbinsX()

        assert len(R_i) == ny, "R array length must match number of IR bins"

        for iy in range(1, ny + 1):  # ROOT bins start at 1 and go to N
            factor = R_i[iy - 1]
            for ix in range(1, nx + 1):
                c = hist_rec_2d_rescaled.GetBinContent(ix, iy)
                c *= factor
                hist_rec_2d_rescaled.SetBinContent(ix, iy, c)
        ...
        if (self.efficiencyFine.FindBin(self.ptBinsArray[-1]) <= self.efficiencyFine.GetNbinsX()):
            nFineBins = self.efficiencyFine.FindBin(self.ptBinsArray[-1])
            histTemp = r.TH1F("efficiencyFine", "efficiencyFine", nFineBins, self.ptBinsArray[0], self.ptBinsArray[-1])
            self.efficiencyFine = histTemp

        for i in range(self.efficiencyFine.GetNbinsX()):
            num = hist_rec_2d_rescaled.Integral(i+1, i+1, 1, -1)
            den = hist_gen_2d.Integral(i+1, i+1, 1, -1)
            # hist_gen_ir, hist_rec_ir, n_trimmed = utils.trim_trailing_zeros_root(hist_gen_ir_orig, hist_rec_ir_orig)
            self.efficiencyFine.SetBinContent(i+1, num / den)
            self.efficiencyFine.SetBinError(i+1, 0)


        # For comparison purposes, calculate the efficiency without IR rescaling
        self.histD0PtMatchedFinalBins = self.histD0PtMatched.Rebin(len(self.ptBinsArray) - 1, f"PtMcMatchedFinalBins", np.asarray(self.ptBinsArray, 'd'))
        self.histD0PtGeneratedFinalBins = self.histD0PtGenerated.Rebin(len(self.ptBinsArray) - 1, f"projPtMcGenFinalBins_y_{self.minY}_{self.maxY}", np.asarray(self.ptBinsArray, 'd'))
        self.efficiencyWithoutIrRescaling = self.histD0PtMatchedFinalBins.Clone()
        self.efficiencyWithoutIrRescaling.SetName("efficiencyWithoutIrRescaling")
        self.efficiencyWithoutIrRescaling.SetTitle("Reconstructed / Generated;p_{T};Ratio")
        self.efficiencyWithoutIrRescaling.Divide(self.efficiencyWithoutIrRescaling, self.histD0PtGeneratedFinalBins, 1, 1, 'B')

        # Plot
        colors = utils.create_seaborn_palette(default_palette)
        if hasattr(self, 'canvasIrCorrected'):
            del self.canvasIrCorrected
        self.canvasIrCorrected = r.TCanvas("canvasIrCorrected", "canvasIrCorrected", 2000, 500)
        self.canvasIrCorrected.Divide(3, 1)
        self.canvasIrCorrected.cd(1)
        self.efficiencyFine.SetStats()
        self.efficiencyFine.Draw('hist')
        self.efficiencyFine.SetTitle(f"{self.efficiency.GetTitle()} (IR dep. corrected)")
        self.efficiencyFine.GetYaxis().SetTitle("Efficiency")
        self.canvasIrCorrected.cd(2)
        self.efficiencyWithoutIrRescaling.SetStats(0)
        self.efficiencyWithoutIrRescaling.SetTitle("")
        self.efficiencyWithoutIrRescaling.GetYaxis().SetTitle("A #times #varepsilon")
        self.efficiencyWithoutIrRescaling.GetXaxis().SetTitle("p_{T} (GeV/c)")
        self.efficiencyWithoutIrRescaling.SetMinimum(0.0)
        self.efficiencyWithoutIrRescaling.SetStats(0)
        self.efficiencyWithoutIrRescaling.SetLineColor(colors[1].GetNumber())
        self.efficiencyWithoutIrRescaling.SetMarkerColor(colors[1].GetNumber())
        self.efficiencyWithoutIrRescaling.SetMarkerStyle(marker_styles[1])
        self.efficiencyWithoutIrRescaling.Draw('EP')
        self.efficiency.SetLineColor(colors[0].GetNumber())
        self.efficiency.SetMarkerColor(colors[0].GetNumber())
        self.efficiency.SetMarkerStyle(marker_styles[0])
        self.efficiency.Draw('same EP')
        self.legendEffIrRescaled = r.TLegend(0.45, 0.14, 0.899, 0.27)
        self.legendEffIrRescaled.SetBorderSize(0)
        self.legendEffIrRescaled.SetMargin(0.04)
        self.legendEffIrRescaled.AddEntry(self.efficiency, "A #times#varepsilon", 'p')
        self.legendEffIrRescaled.AddEntry(self.efficiencyWithoutIrRescaling, "A #times#varepsilon without IR rescaling", 'p')
        self.legendEffIrRescaled.Draw()
        self.canvasIrCorrected.cd(3)
        self.histCorrectedSpectrum.SetStats(0)
        self.histCorrectedSpectrum.Draw()
        r.gPad.SetLogy()
        self.canvasIrCorrected.Draw()

    def apply_zdc_correction(self, correction_factors):
        """
        Apply bin-by-bin ZDC corrections given by the factors in correction_factors.
        Replaces the corrected spectrum.
        """
        if hasattr(self, 'histCorrectedSpectrumBeforeZdcCorrection'):
            raise Exception("ZDC correction was already applied for this analysis!")
        self.histCorrectedSpectrumBeforeZdcCorrection = self.histCorrectedSpectrum.Clone()
        for i, f in enumerate(correction_factors):
            self.histCorrectedSpectrum.SetBinContent(i+1, f * self.histCorrectedSpectrum.GetBinContent(i+1))

    def draw_zdc_correction(self):
        colors = utils.create_seaborn_palette(default_palette)
        if hasattr(self, 'canvasZdcCorrection'):
            del self.canvasZdcCorrection
        self.canvasZdcCorrection = r.TCanvas("canvasZdcCorrection")
        self.canvasZdcCorrection.cd()
        self.histCorrectedSpectrumBeforeZdcCorrection.Draw()
        self.histCorrectedSpectrumBeforeZdcCorrection.SetLineColor(colors[1].GetNumber())
        self.histCorrectedSpectrumBeforeZdcCorrection.SetMarkerStyle(marker_styles[1])
        self.histCorrectedSpectrumBeforeZdcCorrection.SetMarkerColor(colors[1].GetNumber())
        self.histCorrectedSpectrum.Draw('same')
        self.histCorrectedSpectrum.SetLineColor(colors[0].GetNumber())
        self.histCorrectedSpectrum.SetMarkerStyle(marker_styles[0])
        self.histCorrectedSpectrum.SetMarkerColor(colors[0].GetNumber())
        self.legendZdcCorrection = r.TLegend(0.6, 0.75, 0.899, 0.89)
        self.legendZdcCorrection.SetBorderSize(0)
        self.legendZdcCorrection.SetMargin(0.04)
        self.legendZdcCorrection.AddEntry(self.histCorrectedSpectrumBeforeZdcCorrection, "Before ZDC correction", 'p')
        self.legendZdcCorrection.AddEntry(self.histCorrectedSpectrum, "After ZDC correction", 'p')
        self.legendZdcCorrection.Draw()
        r.gPad.SetLogy()
        self.canvasZdcCorrection.Draw()
    ...
    def draw_fits_and_yield(self, showFitRangeOnly=False, nCols=3, showRawYield=True, showLegend=False, componentStyle=1, prependTitle="", width=400, height=333, ymins=None):
        ...
        for i, bin in enumerate(self.ptBins):
            ...
            bin.nominalFitResult.histogram.Draw("E")
            bin.nominalFitResult.histogram.SetTitle(prependTitle + f"{bin.lowerPt} #leq p_{{ T}} < {bin.upperPt} GeV/c")
            bin.nominalFitResult.histogram.GetXaxis().SetTitle("m_{K#pi} (GeV/c^{2})")
            bin.nominalFitResult.histogram.GetYaxis().SetTitle(f"Counts per {bin.nominalFitResult.histogram.GetBinWidth(1)*1000:.0f} MeV/c^{{2}}")
            bin.nominalFitResult.histogram.GetXaxis().SetTitleSize(0.05)
            bin.nominalFitResult.histogram.GetYaxis().SetTitleSize(0.05)
            bin.nominalFitResult.histogram.GetXaxis().SetLabelSize(0.04)
            bin.nominalFitResult.histogram.GetYaxis().SetLabelSize(0.04)
            bin.nominalFitResult.histogram.SetLineColor(r.kBlack)
            bin.nominalFitResult.histogram.SetMarkerStyle(r.kFullCircle)
            bin.nominalFitResult.histogram.SetMarkerSize(0.5)
            bin.nominalFitResult.histogram.SetStats(0)
            bin.nominalFitResult.histogram.SetMinimum(ymins[i] if ymins is not None else 0.0)
            if showFitRangeOnly:
                bin.nominalFitResult.histogram.GetXaxis().SetRangeUser(bin.nominalFitResult.lowerMass, bin.nominalFitResult.upperMass)
            else:
                bin.nominalFitResult.histogram.GetXaxis().SetRangeUser(1.5, 2.2)
            self.legendsFitsYields[i].AddEntry(bin.nominalFitResult.histogram, "Data")
            ...
            bin.nominalFitResult.histogram.Draw("E same")
            if showLegend:
